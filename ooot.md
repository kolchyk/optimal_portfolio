...
def build_adaptive_schedule_space(
    total_days: int,
    *,
    n_is_values: int = 5,
    n_oos_values: int = 4,
) -> dict[str, dict]:
    """Build WFO schedule search space scaled to available trading days.

    IS window ranges from ~4 % to ~25 % of total data.
    OOS window ranges from ~2 % to ~10 % of total data.
    Values are rounded to multiples of 5 for clean grid points.
    """
    is_low = max(20, _round_to(total_days * 0.04, 5))
    is_high = _round_to(total_days * 0.25, 5)
    is_step = max(5, _round_to((is_high - is_low) / max(1, n_is_values - 1), 5))

    oos_low = max(10, _round_to(total_days * 0.02, 5))
    oos_high = _round_to(total_days * 0.10, 5)
    oos_step = max(5, _round_to((oos_high - oos_low) / max(1, n_oos_values - 1), 5))

    return {
        "min_is_days": {"type": "int", "low": is_low, "high": is_high, "step": is_step},
        "oos_days": {"type": "int", "low": oos_low, "high": oos_high, "step": oos_step},
    }
...
def _generate_grid(
    ...
    """Generate all (min_is_days, oos_days) combos from a schedule space.

    If *total_days* is given, combos where ``is + oos`` would not leave room
    for at least *min_wfo_steps* are excluded up-front to save compute.
    """
    is_spec = space["min_is_days"]
    oos_spec = space["oos_days"]
    is_values = list(range(is_spec["low"], is_spec["high"] + 1, is_spec["step"]))
    oos_values = list(range(oos_spec["low"], oos_spec["high"] + 1, oos_spec["step"]))
    combos = [(is_d, oos_d) for is_d in is_values for oos_d in oos_values]

    if total_days is not None:
        max_window = total_days // min_wfo_steps
        combos = [(is_d, oos_d) for is_d, oos_d in combos
                  if is_d + oos_d <= max_window]
...
def run_walk_forward_grid(
    ...
    """Grid search over (min_is_days, oos_days) WFO schedule parameters.
    ...
    schedule_space = schedule_space or build_adaptive_schedule_space(total_days)
    combos = _generate_grid(schedule_space, total_days=total_days)
    ...
    try:
        for idx, (is_days, oos_d) in enumerate(combos):
            log.info(
                "wfo_grid_combo",
    ...
    if not entries:
        raise ValueError("WFO grid search: no valid (min_is_days, oos_days) combination.")
    ...
    log.info(
        "wfo_grid_done",
        best_min_is_days=best.min_is_days,
...
def format_wfo_grid_report(grid_result: WFOGridResult) -> str:
    ...
    lines.append("-" * 90)
    header = (
        f"  {'IS days':>8}  {'OOS days':>9}  {'Steps':>5}"
    ...
    for _, row in summary.iterrows():
        ...
        lines.append(
            f"  {int(row['min_is_days']):>8}  {int(row['oos_days']):>9}  {int(row['n_steps']):>5}"
    ...
    lines.append(f"  min_is_days = {best.min_is_days}")
...
```
Error reading src/portfolio_sim/__pycache__/config.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa0 in position 10: invalid start byte
```
...
```
Error reading src/portfolio_sim/__pycache__/data.cpython-314.pyc: 'utf-8' codec can't decode byte 0xda in position 8: invalid continuation byte
```
...
```
Error reading src/portfolio_sim/__pycache__/reporting.cpython-314.pyc: 'utf-8' codec can't decode byte 0xfa in position 8: invalid start byte
```
## File: portfolio_sim/__pycache__/indicators.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/indicators.cpython-314.pyc: 'utf-8' codec can't decode byte 0x96 in position 8: invalid start byte
```
## File: portfolio_sim/__pycache__/optimizer.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/optimizer.cpython-314.pyc: 'utf-8' codec can't decode byte 0x96 in position 8: invalid start byte
```
## File: portfolio_sim/__pycache__/params.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/params.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa0 in position 10: invalid start byte
```
...
```
Error reading src/portfolio_sim/__pycache__/models.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa0 in position 10: invalid start byte
```
## File: portfolio_sim/__pycache__/parallel.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/parallel.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa9 in position 8: invalid start byte
```
...
```
Error reading src/portfolio_sim/__pycache__/alpha.cpython-314.pyc: 'utf-8' codec can't decode byte 0xb1 in position 8: invalid start byte
```
...
```
Error reading src/portfolio_sim/__pycache__/max_profit.cpython-314.pyc: 'utf-8' codec can't decode byte 0x96 in position 8: invalid start byte
```
## File: portfolio_sim/__pycache__/engine.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/engine.cpython-314.pyc: 'utf-8' codec can't decode byte 0xc5 in position 8: invalid continuation byte
```
## File: portfolio_sim/__pycache__/walk_forward.cpython-314.pyc
```
Error reading src/portfolio_sim/__pycache__/walk_forward.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa0 in position 10: invalid start byte
```
...
```
Error reading src/portfolio_sim/commands/__pycache__/max_profit.cpython-314.pyc: 'utf-8' codec can't decode byte 0x96 in position 8: invalid start byte
```
## File: portfolio_sim/commands/__pycache__/walk_forward.cpython-314.pyc
```
Error reading src/portfolio_sim/commands/__pycache__/walk_forward.cpython-314.pyc: 'utf-8' codec can't decode byte 0xa0 in position 10: invalid start byte
```
