...
not for ranking momentum.
    ...
    st.markdown(
        f"""
An asset enters the portfolio through three sequential steps:
...
> their own trend holds \u2014 true cross-asset diversification.
    ...
    st.subheader("Step 2 \u2014 R\u00b2 Momentum Scoring & Top-N Selection")
    st.markdown(
        f"""
...
For each asset, we fit an OLS regression to the log of its price over a
`{_DEFAULTS.r2_window}`-day lookback window:
...
> looks like a straight line gets priority over the one that zigzags wildly.
    ...
    st.subheader("Step 3 \u2014 Per-Class Diversification Limits")
    st.markdown(
        f"""
After ranking by R\u00b2 score, candidates are subject to **per-class position limits**.

Each asset class (US Equity, Bonds, Metals, Crypto, etc.) can have at most
**{_DEFAULTS.max_per_class}** positions in the portfolio simultaneously.

This prevents the portfolio from becoming overly concentrated in a single
asset class, even if that class has the strongest momentum.

> *In plain English:* even if all the top-scoring assets are tech stocks,
> we won't hold more than {_DEFAULTS.max_per_class} of them \u2014 we force
...
- **KAMA stop (daily):** closing price < KAMA \u00d7 (1 \u2212 {_BUFFER_PCT:.1f}%)
- **Rotation stop (on rebalance):** asset falls outside the Top-{_DEFAULTS.top_n * 2}
  extended ranking \u2014 its R\u00b2 score dropped, it failed the KAMA filter, or stronger
  candidates pushed it out

A position is **NOT** sold just because it slipped from Top-{_DEFAULTS.top_n} to,
say, position {_DEFAULTS.top_n + 3}. As long as it stays within the extended
Top-{_DEFAULTS.top_n * 2} list and its KAMA trend holds, we keep it \u2014 this is
the "lazy hold" mechanism.

Rebalancing (signal check) happens **monthly** (~{_DEFAULTS.rebal_days} trading days),
...
Investment floor: **{_DEFAULTS.min_invested_pct:.0%}** (0% = disabled).

If the market is calm, positions increase.
If volatile, they shrink automatically.
When enabled, the investment floor prevents vol-targeting from
reducing exposure below the set minimum.

> **Analogy:** autopilot that keeps the "speed of risk" steady,
> with an optional minimum cruising speed.
            """
        )
        ...
        st.table(
            {
                ...
                "Default": [
                    ...
                    f"{_DEFAULTS.r2_window} days",
                    f"{_DEFAULTS.kama_asset_period} days",
                    f"\u00b1{_BUFFER_PCT:.1f}%",
                    f"{_DEFAULTS.top_n} assets",
                    f"{_DEFAULTS.rebal_days} days (monthly)",
        ...
        st.table(
            {
                ...
                "Default": [
                    ...
                    f"{_DEFAULTS.min_invested_pct:.0%}",
                ],
                "What It Does": [
                    ...
                    "Floor for invested fraction (overrides vol-targeting down-scaling)",
                ],
            }
        )
...
| Step | Timing | Action |
|------|--------|--------|
| 1 | Monthly (~{_DEFAULTS.rebal_days} trading days), close of day T | Analyze signals: asset KAMA filter, R\u00b2 scoring |
...
> (except for vol-targeting scaling and daily KAMA-break exits). This reduces turnover.
        """
    )
...
**Objective function:** CAGR (maximize) with MaxDD \u2264 {MAX_DD_LIMIT:.0%} constraint.
...
`uv run python -m src.portfolio_sim walk-forward`
    ...
    # 10. Advantages
    # ------------------------------------------------------------------
    st.header("10. Advantages of This Approach")
    st.markdown(
        """
...
- **Low turnover.** Monthly Lazy-Hold rebalancing reduces the number
...
- **Rebalancing** \u2014 the periodic process of reviewing and adjusting the
  portfolio. We do it monthly (~{_DEFAULTS.rebal_days} trading days).
...
а не для ранжування моментуму.
    ...
    st.markdown(
        f"""
Актив потрапляє в портфель через три послідовних кроки:
...
> залишитися, якщо їхній тренд тримається — справжня міжкласова диверсифікація.
    ...
    st.subheader("Крок 2 — R² Momentum Scoring та вибір Top-N")
    st.markdown(
        f"""
...
Для кожного активу ми будуємо OLS-регресію логарифму його ціни за вікно
`{_DEFAULTS.r2_window}` днів:
...
> виглядає як пряма лінія, має пріоритет над тим, що рухається зигзагами.
    ...
    st.subheader("Крок 3 — Ліміти диверсифікації по класах")
    st.markdown(
        f"""
Після ранжування за R² score кандидати підлягають **лімітам позицій по класах**.

Кожен клас активів (US Equity, Bonds, Metals, Crypto тощо) може мати не більше
**{_DEFAULTS.max_per_class}** позицій у портфелі одночасно.

Це запобігає надмірній концентрації портфеля в одному класі активів,
навіть якщо цей клас має найсильніший моментум.

> *Простою мовою:* навіть якщо всі найкращі активи — це технологічні акції,
> ми не тримаємо більше {_DEFAULTS.max_per_class} з них — ми примусово
> диверсифікуємо між класами активів.
        """
    )
...
- **KAMA стоп (щодня):** ціна закриття < KAMA × (1 − {_BUFFER_PCT:.1f}%)
- **Rotation stop (на ребалансуванні):** актив випав за межі Top-{_DEFAULTS.top_n * 2}
  розширеного рейтингу — його R² score впав, не пройшов KAMA-фільтр,
  або сильніші кандидати витіснили його

Позиція **НЕ** продається лише тому, що вона просіла з Top-{_DEFAULTS.top_n} на,
скажімо, позицію {_DEFAULTS.top_n + 3}. Поки вона залишається в розширеному
Top-{_DEFAULTS.top_n * 2} списку і її KAMA-тренд тримається, ми її зберігаємо —
це механізм "lazy hold".

Ребалансування (перевірка сигналів) відбувається **щомісячно** (~{_DEFAULTS.rebal_days} торгових днів),
...
Мінімум інвестицій: **{_DEFAULTS.min_invested_pct:.0%}** (0% = вимкнено).

Якщо ринок спокійний — позиції збільшуються.
Якщо волатильний — автоматично зменшуються.
Коли увімкнено, мінімальний поріг інвестицій не дає
vol-targeting зменшити експозицію нижче встановленого рівня.

> **Аналогія:** автопілот, що тримає "швидкість ризику" стабільною,
> з опціональною мінімальною крейсерською швидкістю.
            """
        )
        ...
        st.table(
            {
                ...
                "За замовчуванням": [
                    "3 роки",
                    f"${INITIAL_CAPITAL:,.0f}",
                    f"{_DEFAULTS.r2_window} днів",
                    f"{_DEFAULTS.kama_asset_period} днів",
                    f"±{_BUFFER_PCT:.1f}%",
                    f"{_DEFAULTS.top_n} активів",
                    f"{_DEFAULTS.rebal_days} днів (щомісячно)",
                    f"{COMMISSION_RATE:.2%} ({COMMISSION_RATE * 10_000:.0f} bps)",
                    f"{SLIPPAGE_RATE:.2%} ({SLIPPAGE_RATE * 10_000:.0f} bps)",
        ...
        st.table(
            {
                ...
                "Що це робить": [
                    ...
                    "Поріг інвестованої частки (перевизначає зменшення vol-targeting)",
                ],
            }
        )
...
| Крок | Час | Дія |
|------|-----|-----|
| 1 | Щомісячно (~{_DEFAULTS.rebal_days} торг. днів), закриття дня T | Аналіз сигналів: KAMA-фільтр активу, R² scoring |
| 2 | Закриття дня T | Визначити продажі (пробій KAMA, rotation stop) та покупки (нові кандидати top-N) |
...
> (окрім vol-targeting масштабування та щоденних KAMA-break виходів). Це зменшує оборот.
        """
    )
...
**Цільова функція:** CAGR (максимізація) з обмеженням MaxDD ≤ {MAX_DD_LIMIT:.0%}.
...
`uv run python -m src.portfolio_sim walk-forward`
    ...
    # 10. Переваги
    # ------------------------------------------------------------------
    st.header("10. Переваги цього підходу")
    st.markdown(
        """
...
- **Ребалансування** — періодичний процес перегляду та коригування
  портфеля. Ми робимо це щомісячно (~{_DEFAULTS.rebal_days} торгових днів).
