"""S2 Walk-Forward Optimization — hybrid R² Momentum + vol-targeting.

Reuses schedule generation and equity-curve stitching from the base
walk_forward module.  Uses S2 engine, optimizer, and params.
"""
...
from src.portfolio_sim.strategy_s2.config import (
    S2_MAX_DD_LIMIT,
    S2_SEARCH_SPACE,
    get_s2_kama_periods,
)
from src.portfolio_sim.strategy_s2.engine import run_simulation_s2
from src.portfolio_sim.strategy_s2.optimizer import (
    find_best_params_s2,
    run_sensitivity_s2,
)
from src.portfolio_sim.strategy_s2.parallel import init_eval_worker_s2
from src.portfolio_sim.strategy_s2.params import S2StrategyParams

log = structlog.get_logger()


def run_walk_forward_s2(
    close_prices: pd.DataFrame,
    open_prices: pd.DataFrame,
    tickers: list[str],
    initial_capital: float = INITIAL_CAPITAL,
    base_params: S2StrategyParams | None = None,
    space: dict[str, dict] | None = None,
    n_trials_per_step: int = 100,
    n_workers: int | None = None,
    min_is_days: int | None = None,
    oos_days: int | None = None,
    max_dd_limit: float = S2_MAX_DD_LIMIT,
    metric: str = "total_return",
    kama_caches: dict[int, dict[str, pd.Series]] | None = None,
    executor: ProcessPoolExecutor | None = None,
    verbose: bool = True,
) -> WFOResult:
    """Run sliding walk-forward optimisation with S2 hybrid engine."""
    base_params = base_params or S2StrategyParams()
    ...
    if not schedule:
        raise ValueError(
            f"Not enough data for S2 WFO: {len(dates)} trading days available, "
    ...
    if kama_caches is None:
        kama_periods = get_s2_kama_periods(space)
        kama_caches = precompute_kama_caches(
            close_prices, tickers, kama_periods, n_workers,
        )
        log.info("s2_wfo_kama_precomputed", n_periods=len(kama_caches))
    ...
    if own_executor:
        executor = ProcessPoolExecutor(
            ...
            initializer=init_eval_worker_s2,
    ...
    try:
        for step_idx, (is_start, is_end, oos_start, oos_end) in enumerate(schedule):
            ...
            sens_result = run_sensitivity_s2(
            ...
            best_params = find_best_params_s2(sens_result)
            if best_params is None:
                log.warning("s2_wfo_step_no_valid_params", step=step_idx + 1)
            ...
            if oos_equity.empty:
                log.warning("s2_wfo_step_empty_oos", step=step_idx + 1)
                continue
    ...
    if not steps:
        raise ValueError("S2 WFO produced no valid steps.")
...
# ---------------------------------------------------------------------------
def run_s2_schedule_optimization(
    close_prices: pd.DataFrame,
    open_prices: pd.DataFrame,
    tickers: list[str],
    initial_capital: float = INITIAL_CAPITAL,
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("S2 SCHEDULE OPTIMIZATION RESULTS")
        print("=" * 80)
        ...
        best_result = run_walk_forward_s2(
            close_prices, open_prices, tickers,
            initial_capital=initial_capital,
            base_params=base_params,
            space=space,
...
# Report formatting
# ---------------------------------------------------------------------------
def format_wfo_report_s2(result: WFOResult) -> str:
    """Format a human-readable S2 walk-forward optimisation report."""
    lines: list[str] = []
    lines.append("=" * 90)
    lines.append("WALK-FORWARD OPTIMIZATION REPORT (S2: Hybrid R\u00b2 Momentum + Vol-Targeting)")
    lines.append("=" * 90)
    ...
    # Recommended live parameters (S2-specific)
