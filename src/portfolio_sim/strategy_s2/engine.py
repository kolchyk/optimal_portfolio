"""S2 hybrid bar-by-bar simulation engine.

Combines the best elements of R2 (Clenow R² Momentum) and V2 (vol-targeting):
...
# Main simulation
# ---------------------------------------------------------------------------
def run_simulation_s2(
    close_prices: pd.DataFrame,
    open_prices: pd.DataFrame,
    tickers: list[str],
    initial_capital: float,
    params: S2StrategyParams | None = None,
    kama_cache: dict[str, pd.Series] | None = None,
    spy_kama_ext: pd.Series | None = None,
    show_progress: bool = False,
) -> SimulationResult:
    """Run S2 hybrid bar-by-bar simulation.
    ...
    # ------------------------------------------------------------------
    date_iter = (
        tqdm(sim_dates, desc="Simulating S2", unit="day")
        if show_progress
        else sim_dates
    )
    for i, date in enumerate(date_iter):
        ...
        # --- Vol-targeting trim on Open (from V2) ---
