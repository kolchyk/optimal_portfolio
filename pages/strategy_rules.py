...
- **Period for SPY (regime filter):** `{_DEFAULTS.kama_spy_period}` trading days
...
not for ranking momentum.
    ...
    st.markdown(
        f"""
An asset enters the portfolio through five sequential steps:

**Step 1 \u2014 SPY Regime Filter:**
SPY price > KAMA(SPY) \u00d7 (1 \u2212 {_BUFFER_PCT:.1f}%). If the overall market is in
"bear" mode, the entire portfolio moves to cash.

> *In plain English:* if the S&P 500 is falling below its trend line,
> we don't buy anything. We wait for better times.

**Step 2 \u2014 Asset KAMA Filter:**
Asset closing price > KAMA \u00d7 (1 \u2212 {_BUFFER_PCT:.1f}%). The asset must be
above its own trend.

> *In plain English:* each asset must be in an uptrend to be considered.

**Step 3 \u2014 Gap Filter:**
No single-day move > {_GAP_PCT:.0f}% in the last `{_DEFAULTS.r2_lookback}` days.
...
New entries are checked for correlation with current holdings. If
correlation > {_DEFAULTS.corr_threshold}, the asset is not added \u2014 this
ensures portfolio diversification.

> *In plain English:* we don't want too many similar assets.
> If two stocks move together, holding both doesn't reduce risk.
        """
    )

    st.subheader("Step 5 \u2014 R\u00b2 Momentum Scoring & Top-N Selection")
    st.markdown(
        f"""
...
For each asset, we fit an OLS regression to the log of its price over the last
`{_DEFAULTS.r2_lookback}` days:
...
- **SPY regime off:** SPY falls below its KAMA \u2014 entire portfolio goes to cash
        ...
        st.table(
            {
                ...
                "Default": [
                    ...
                    f"{_DEFAULTS.r2_lookback} days",
                    f"{_DEFAULTS.kama_asset_period} days",
                    f"{_DEFAULTS.kama_spy_period} days",
                    f"\u00b1{_BUFFER_PCT:.1f}%",
        ...
        st.table(
            {
                ...
                "What It Does": [
                    ...
                    "Correlation filter for new entries",
...
| Step | Timing | Action |
|------|--------|--------|
| 1 | Every {_DEFAULTS.rebal_period_weeks} weeks, close of day T | Analyze signals: SPY KAMA, asset KAMA, gap filter, R\u00b2 scoring |
...
- **Protection from large drawdowns.** The SPY KAMA regime filter moves the
  entire portfolio to cash during bear markets.

- **Cross-asset diversification + correlation filter.** The portfolio includes
  not just stocks, but also bonds, gold, and real estate.
  The correlation filter prevents excessive concentration.
...
- **Calmar Ratio** \u2014 annual return divided by maximum drawdown.
  Higher is better.
- **Correlation Filter** \u2014 checks correlation between a new candidate
  and current holdings. Prevents excessive concentration in similar assets.
...
- **Період для SPY (фільтр режиму):** `{_DEFAULTS.kama_spy_period}` торгових днів
...
а не для ранжування моментуму.
    ...
    st.markdown(
        f"""
Актив потрапляє в портфель через п'ять послідовних кроків:

**Крок 1 — Фільтр режиму SPY:**
Ціна SPY > KAMA(SPY) × (1 − {_BUFFER_PCT:.1f}%). Якщо загальний ринок у
"ведмежому" режимі, весь портфель переходить у кеш.

> *Простою мовою:* якщо S&P 500 падає нижче своєї лінії тренду,
> ми нічого не купуємо. Чекаємо на кращі часи.

**Крок 2 — Фільтр KAMA активу:**
Ціна закриття активу > KAMA × (1 − {_BUFFER_PCT:.1f}%). Актив повинен бути
вище свого власного тренду.

> *Простою мовою:* кожен актив має бути у висхідному тренді, щоб його розглядали.

**Крок 3 — Фільтр гепів:**
Жодного одноденного руху > {_GAP_PCT:.0f}% за останні `{_DEFAULTS.r2_lookback}` днів.
...
**Крок 4 — Фільтр кореляції:**
Нові кандидати перевіряються на кореляцію з поточними позиціями. Якщо
кореляція > {_DEFAULTS.corr_threshold}, актив не додається — це
забезпечує диверсифікацію портфеля.

> *Простою мовою:* ми не хочемо надто багато схожих активів.
> Якщо дві акції рухаються разом, тримати обидві не зменшує ризик.
        """
    )

    st.subheader("Крок 5 — R² Momentum Scoring та вибір Top-N")
    st.markdown(
        f"""
...
Для кожного активу ми будуємо OLS-регресію логарифму його ціни за останні
`{_DEFAULTS.r2_lookback}` днів:
...
- **Режим SPY вимкнено:** SPY падає нижче свого KAMA — весь портфель переходить у кеш
        ...
        st.table(
            {
                ...
                "За замовчуванням": [
                    "3 роки",
                    f"${INITIAL_CAPITAL:,.0f}",
                    f"{_DEFAULTS.r2_lookback} днів",
                    f"{_DEFAULTS.kama_asset_period} днів",
                    f"{_DEFAULTS.kama_spy_period} днів",
                    f"±{_BUFFER_PCT:.1f}%",
        ...
        st.table(
            {
                ...
                "Що це робить": [
                    ...
                    "Фільтр кореляції для нових входів",
...
| Крок | Час | Дія |
|------|-----|-----|
| 1 | Кожні {_DEFAULTS.rebal_period_weeks} тижні, закриття дня T | Аналіз сигналів: SPY KAMA, KAMA активу, фільтр гепів, R² scoring |
...
- **Захист від великих просадок.** Фільтр режиму SPY KAMA переводить
  весь портфель у кеш під час ведмежих ринків.

- **Міжкласова диверсифікація + фільтр кореляції.** Портфель включає
  не лише акції, а й облігації, золото та нерухомість.
  Фільтр кореляції запобігає надмірній концентрації.
...
- **Calmar Ratio** — річна дохідність, поділена на максимальну просадку.
  Чим вище — тим краще.
- **Фільтр кореляції** — перевіряє кореляцію між новим кандидатом
  та поточними позиціями. Запобігає надмірній концентрації у схожих активах.
