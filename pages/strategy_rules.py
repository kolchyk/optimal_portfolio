...
def page():
    ...
    st.caption(
        "Long/Cash | S&P 500 або Cross-Asset ETF | "
...
Ніколи не "шортимо" (не граємо на зниження).
    ...
    st.info(
        "**Два режими роботи:**\n\n"
        "**S&P 500** — ~500 акцій найбільших компаній США. "
        "Концентрований моментум з ринковим фільтром SPY.\n\n"
    ...
    # ------------------------------------------------------------------
    # 2. Two asset universes
    # ------------------------------------------------------------------
    st.header("2. Два всесвіти активів")

    col_sp, col_etf = st.columns(2)

    with col_sp:
        st.subheader("S&P 500")
        st.markdown(
            """
**~500 найбільших публічних компаній США.**

- Однорідний всесвіт: тільки акції
- SPY як бенчмарк та фільтр ринкового режиму
- Рівна вага позицій (equal weight)
- Ранжування за сирим моментумом

> Найкраще підходить для агресивного
> концентрованого моментуму на одному ринку.
            """
        )
...
> облігації та золото часто зростають.
    ...
    st.markdown(
        """
> **Аналогія:** S&P 500 — це як обирати найкращих спортсменів
> в одному виді спорту. Cross-Asset ETF — це як збирати олімпійську
...
загалом в низхідному тренді.
    ...
    tab_raw, tab_risk = st.tabs(
        ["Сирий моментум (S&P 500)", "Ризик-скоригований (ETF)"]
    )

    with tab_raw:
        st.markdown(
            f"""
**За замовчуванням у режимі S&P 500.**

Формула: `score = (ціна_зараз / ціна_{LOOKBACK_PERIOD}_днів_тому) − 1`

З усіх кандидатів обираємо **Топ-{TOP_N}** з найвищим score і
розподіляємо капітал порівну (~{100 / TOP_N:.0f}% на позицію).

**Приклад:** якщо 150 акцій пройшли KAMA-фільтр, ранжуємо їх за
6-місячною дохідністю і беремо лише {TOP_N} найкращих.
    ...
    with tab_risk:
        st.markdown(
            f"""
**За замовчуванням у режимі ETF Cross-Asset.**
...
> але стабільно і надійно. Ризик-скоригований метод обере другого.
    ...
    st.markdown(
        f"""
**За замовчуванням:** вимкнений у S&P 500, увімкнений у ETF.
...
**Equal Weight** (S&P 500 за замовч.): кожна позиція — строго
...
- **Захист від великих просадок.** У ведмежому ринку портфель
  переходить у кеш (S&P 500 режим) або ротує в некорельовані активи
