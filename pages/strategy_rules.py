...
- **Період:** `{KAMA_PERIOD}` торгових днів (приблизно 1 місяць)
...
2. **Позитивний моментум:** дохідність за останні `{LOOKBACK_PERIOD}` днів
   (приблизно 1 квартал) має бути додатною.
...
3-місячною дохідністю і беремо лише 20 найкращих.
        """
    )
    ...
    st.table(
        {
            ...
            "Значення": [
                ...
                f"{KAMA_PERIOD} днів",
                f"{LOOKBACK_PERIOD} днів (~1 квартал)",
                f"{TOP_N} акцій",
                f"±{_BUFFER_PCT:.0f}%",
