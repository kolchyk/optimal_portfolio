"""Strategy rules explanation page — Hybrid R² Momentum + Vol-Targeting (Ukrainian, beginner-friendly)."""
...
Андреаса Кленова ("Stocks on the Move") з **vol-targeting overlay**.

**Ключова ідея:** ми фітимо лінійну регресію до логарифму ціни за останні
...
Поверх ATR risk parity позиціонування додається **vol-targeting overlay**:
портфель масштабується так, щоб реалізована волатильність наближалась
до цільового рівня (target_vol). Це автоматично зменшує експозицію
в турбулентні періоди і збільшує в спокійні.
...
- **Період для SPY (режимний фільтр):** `{_DEFAULTS.kama_spy_period}` торгових днів
...
В стратегії KAMA використовується як **тренд-фільтр**
...
**Крок 4 — Кореляційний фільтр:**
Нові входи перевіряються на кореляцію з поточними позиціями. Якщо
кореляція > {_DEFAULTS.corr_threshold}, актив не додається — це
забезпечує диверсифікацію портфеля.
        """
    )

    st.subheader("Крок 5 — R² Momentum скоринг і відбір Top-N")
    st.markdown(
        f"""
...
Відбираємо **Top-{_DEFAULTS.top_n}** активів з найвищим score > 0.
...
- **Gap exit:** одноденний рух > {_GAP_PCT:.0f}% — негайний вихід
- **R² Score < 0** або актив не пройшов фільтри на дату ребалансу

Позиція **НЕ** продається лише тому, що актив випав з Топ-{_DEFAULTS.top_n}
за моментумом. Поки його власний тренд тримається — ми тримаємо позицію.

Ребалансування (перевірка сигналів) відбувається кожні
**{_DEFAULTS.rebal_period_weeks} тижні**, а не щодня. Це зменшує оборотність
і комісійні витрати.
...
Портфель масштабується щоденно:
...
Якщо волатильний — зменшуються автоматично.
        ...
        st.table(
            {
                ...
                "Пояснення": [
                    ...
                    "Максимальний масштабний коефіцієнт",
                    "Вікно оцінки реалізованої волатильності",
                ],
            }
        )
...
| Крок | Час | Дія |
|------|-----|-----|
| 1 | Кожні {_DEFAULTS.rebal_period_weeks} тижні, закриття дня T | Аналізуємо сигнали: SPY KAMA, asset KAMA, gap filter, R² scoring |
...
| 4 | Кожен день | Vol-targeting overlay: масштабування позицій за target_vol / realised_vol |
| 5 | Кожен день | Gap exit: негайний вихід якщо |daily_return| > порога |
...
> (за винятком vol-targeting та gap exits). Це знижує оборотність.
        """
    )
...
> Ви можете їхати повільніше, але не швидше без збільшення ризику.
    ...
    st.markdown(
        """
**Чому гібридна стратегія краща за наївний "купи і тримай"?**
...
- **Vol-targeting overlay.** Автоматичне масштабування портфеля
  до цільової волатильності. В турбулентні періоди експозиція
  зменшується, в спокійні — збільшується.

- **Захист від великих просадок.** SPY KAMA режимний фільтр переводить
  весь портфель у кеш при ведмежому ринку.

- **Крос-активна диверсифікація + кореляційний фільтр.** Портфель
  включає не лише акції, а й облігації, золото, нерухомість.
  Кореляційний фільтр запобігає надмірній концентрації.
...
- **Risk Factor** — ризик на позицію в день ({_DEFAULTS.risk_factor * 10_000:.0f} bps).
  Визначає, скільки капіталу виділяється на кожен актив відносно його ATR.
- **Vol-Targeting** — overlay, що масштабує портфель до цільової
...
- **KAMA** — адаптивна ковзна середня, що реагує на силу тренду.
  Розроблена Перрі Кауфманом. В стратегії використовується як
  тренд-фільтр.
- **Гістерезис** — буфер, що запобігає частим хибним перемиканням.
- **Gap (геп)** — різкий одноденний стрибок ціни (>{_GAP_PCT:.0f}%).
...
- **Кореляційний фільтр** — перевіряє кореляцію між кандидатом
  і поточними позиціями. Запобігає надмірній концентрації.
            """
        )
