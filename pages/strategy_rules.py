...
_Детальніше — у секції 5, пункт А._
            """
        )
...
> більше ризику, менше грошей. Кожен "ризикує" однаково.
    ...
    with row2_c2:
        ...
        st.markdown(
            f"""
**Диверсифікація за змістом**

Перед додаванням активу перевіряємо його кореляцію з уже обраними.
Якщо зв'язок > `{CORRELATION_THRESHOLD}` — пропускаємо його як занадто схожий.
...
- **Розумне управління ризиками.** Risk Parity та кореляційний фільтр забезпечують, щоб жоден актив не домінував у портфелі та щоб позиції були максимально різноманітними.
