...
загалом в низхідному тренді.
    ...
    st.markdown(
        f"""
**Ранжування за ER-скоригованим моментумом (Cascade-style).**

Формула: `score = дохідність × ER²` (Efficiency Ratio squared)

**ER (Efficiency Ratio)** — це та сама метрика, яку KAMA використовує
всередині для адаптації. Вона вимірює «гладкість» тренду: ER = 1 для
ідеально гладкого тренду, ER = 0 для хаотичного руху.

**Множник ER²** жорстко карає нестабільний рух:
- ER = 0.8 (гладкий тренд) → множник 0.64 — зберігає більшу частину дохідності
- ER = 0.3 (хаотичний рух) → множник 0.09 — майже обнуляє скор

Після ранжування відбираємо **Top-{TOP_N}** активів з найвищим скором.

> **Аналогія:** Два бігуни з однаковим результатом — перевагу
> отримає той, хто біжить рівніше, а не той, хто кидається
> з боку в бік.
        """
    )
        ...
        st.table(
            {
                ...
                "Пояснення": [
                    "Ранжування за return × ER² замість raw return",
                    "Вимагати, щоб KAMA була направлена вгору для входу",
...
- **Ризик-скоригований моментум (Risk-Adjusted Momentum)** —
  ранжування за return × ER² замість сирої дохідності.
  Перевага: жорстко карає хаотичний рух, нормалізує активи
  з різним рівнем ризику.
- **Efficiency Ratio (ER)** — відношення чистого руху ціни до
  сумарної волатильності за період. Вимірює «гладкість» тренду.
  ER = 1 — ідеально гладкий тренд, ER = 0 — хаотичний рух.
  Використовується як множник (ER²) для ранжування моментуму.
